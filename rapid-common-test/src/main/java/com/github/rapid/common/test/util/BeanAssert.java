...
public class BeanAssert {
	...
	public static void testPropertiesAndCommonMethod(Class<?> clazz) throws InstantiationException, IllegalAccessException {
		...
		if(bean.equals(origian)) throw new AssertionError("equals() method error,"+bean.equals(origian));
		if((bean.hashCode() == origian.hashCode())) throw new AssertionError("hashCode() method error");
		if((bean.toString().equals(origian.toString()))) throw new AssertionError("toString() method error");
	}
